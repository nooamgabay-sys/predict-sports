...
import { useRealtimeLeagueLeaderboard } from '@/lib/hooks/useRealtimeLeagueLeaderboard'
import type { LeagueLeaderboardEntry } from '@/lib/hooks/useRealtimeLeagueLeaderboard'
import { cn } from '@/lib/utils'
import { Trophy, Star } from 'lucide-react'

interface LeagueLeaderboardClientProps {
  leagueId: string
  initialData: LeagueLeaderboardEntry[]
  currentUserId: string
}

export default function LeagueLeaderboardClient({
  leagueId,
  initialData,
  currentUserId,
}: LeagueLeaderboardClientProps) {
  const { entries } = useRealtimeLeagueLeaderboard(leagueId, initialData)

  if (entries.length === 0) {
    return (
      <div className="glass-card p-10 text-center">
        <Trophy className="w-12 h-12 text-slate-600 mx-auto mb-3" />
        <p className="text-slate-400">No members in this league yet.</p>
      </div>
    )
  }
  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      <div className="grid grid-cols-[3rem_1fr_7rem] sm:grid-cols-[3rem_1fr_8rem_8rem] gap-3 px-4 py-3 border-b border-slate-700/50 text-xs font-semibold text-slate-500 uppercase tracking-wider">
        ...
        <div>Player</div>
        <div className="text-right hidden sm:block">Joined</div>
      ...
      <div className="divide-y divide-slate-700/30">
        {entries.map((entry, index) => {
          ...
          const isTop3 = rank <= 3
          const joinedDate = new Date(entry.joined_at).toLocaleDateString(undefined, {
            month: 'short',
            day: 'numeric',
          })
          ...
          return (
            ...
            >
              ...
              <div className="text-right hidden sm:block">
                <span className="text-sm text-slate-400">{joinedDate}</span>
              </div>
